	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) // login failed
				pw.print("<h4> Invalid Email or Password , Please <a href='index.html'>Retry</a></h4>");
			else {// =>success , (send user details)
				// Create a cookie to hold user details 
				// javax.servlet.http.Cookie(String nm,String val)
				Cookie c1 = new Cookie("usr_dtls", user.toString());
				response.addCookie(c1);
				// add the cookie to to resp header
